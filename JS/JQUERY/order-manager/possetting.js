...
$(document).ready(function () {
    ...
    if (basicinfo.segment4 != null) {
        swal(
            { title: lang.ord_uodate_success, text: lang.do_print_token, type: "success", showCancelButton: true, confirmButtonColor: "#28a745", confirmButtonText: "Yes", cancelButtonText: "No", closeOnConfirm: false, closeOnCancel: true },
            function (isConfirm) {
                if (isConfirm) {
...
$(window).on("load", function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#openregister").modal({ backdrop: "static", keyboard: false });
        },
    });
...
$(document).ready(function () {
    "use strict";
    $("select.form-control:not(.dont-select-me)").select2({ placeholder: lang.sl_option, allowClear: true });
    $("#validate").validate();
    $("#add_category").validate();
    $("#customer_name").validate();
    $(".product-list").slimScroll({ size: "3px", height: "345px", allowPageScroll: true, railVisible: true });
    $(".product-grid").slimScroll({ size: "3px", height: "calc(100vh - 180px)", allowPageScroll: true, railVisible: true });
    $(".cat-grid").slimScroll({ size: "3px", height: "calc(100vh - 180px)", allowPageScroll: true, railVisible: true });
            ...
            processResults: function (data) {
                return {
                    results: $.map(data, function (item) {
                        return { text: item.text + "-" + item.variantName, id: item.id + "-" + item.variantid };
                    }),
                };
            },
            cache: true,
...
function printRawHtml(view) {
    printJS({ printable: view, type: "raw-html" });
}
function placeorder() {
    ...
    if (errormessage == "") {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                var err = data;
                if (err == "error") {
                    swal({ title: lang.ord_failed, text: lang.failed_msg, type: "warning", showCancelButton: true, confirmButtonColor: "#DD6B55", confirmButtonText: "Yes, Cancel!", closeOnConfirm: true }, function () {});
                } else {
                    if (basicinfo.printtype == 1) {
                        swal({ title: lang.ord_succ, text: "", type: "success", showCancelButton: false, confirmButtonColor: "#28a745", confirmButtonText: "Done", closeOnConfirm: true }, function () {});
                    } else {
                        swal(
                            {
                                title: lang.ord_succ,
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (err == "error") {
                    ...
                    swal(
                        { title: lang.ord_places, text: lang.do_print_in, type: "success", showCancelButton: true, confirmButtonColor: "#28a745", confirmButtonText: "Yes", cancelButtonText: "No", closeOnConfirm: true, closeOnCancel: true },
                        function (isConfirm) {
                            if (isConfirm) {
...
function postupdateorder_ajax() {
    ...
    $.ajax({
        ...
        success: function (result) {
            swal({ title: result.msg, text: result.tokenmsg, type: "success", showCancelButton: true, confirmButtonColor: "#28a745", confirmButtonText: "Yes", cancelButtonText: "No", closeOnConfirm: true, closeOnCancel: true }, function (
                isConfirm
            ) {
                if (isConfirm) {
                    $.ajax({
...
function orderconfirmorcancel(status, orderid) {
    ...
    $.ajax({
        type: "POST",
        url: basicinfo.baseurl + "ordermanage/order/changestatus",
        data: dataString,
        success: function (data) {
            $("#onprocesslist").html(data);
            if (mystatus == "9") {
                ...
                swal({ title: lang.ord_complte, text: lang.ord_com_sucs, type: "success", showCancelButton: false, confirmButtonColor: "#28a745", confirmButtonText: "Yes", closeOnConfirm: true }, function () {
                    prevsltab.trigger("click");
                    $("#paidamount").val("");
                    $("#payprint").modal("hide");
                });
    ...
    $.ajax({
        ...
        success: function (printdata) {
            if (basicinfo.printtype != 1) {
                ...
                printJS({ printable: "kotenpr", onPrintDialogClose: printJobComplete, type: "html", font_size: "25px", style: style, scanStyles: false });
            }
        },
    });
}
...
function multi_table() {
    var arr = $('input[name="add_table[]"]:checked')
        .map(function () {
            return this.value;
        })
        .get();
